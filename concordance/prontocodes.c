...
{
    printf("Freeing ir_signal\n");
	free(ir_signal);
    printf("Freed ir_signal\n");
}

void pco_delete_pronto_code(struct pco_pronto_code pronto_code)
{
    printf("Not freeing since it blows up on once signal - tbd\n");
    return;
    printf("Freeing once_signal %x\n", pronto_code.once_signal);
	free(pronto_code.once_signal);
    printf("Freeing repeat_signal %x\n", pronto_code.repeat_signal);
	       ...
	       struct pco_pronto_code *pronto_code)
{
	...
	if (_get_hex_word(&hs_cursor, hs_end, &next_word) != 1 ) {
        printf("BAD first word\n");
		return PCO_ERROR_BAD_CODE;
	}
#ifdef _DEBUG
    printf("Got first word - format_id %4x\n", next_word);
#endif
...
#ifdef _DEBUG
    printf("Got carrier clock divider %4x\n", next_word);
#endif
...
#ifdef _DEBUG
    printf("Returning PCO_RETURN_OK\n");
#endif
	...
	uint32_t repetitions, uint32_t **ir_signal, uint32_t *ir_signal_length)
{
	...
	if (((source.once_signal_length > 0) && (source.once_signal == NULL)) || 
		...
		(carrier_cycle == 0)) {
		printf("source.once_signal_length: %d source.once_signal: %x source.repeat_signal_length: %d source .repeat_signal %x carrier_cycle: %d\n",
		source.once_signal_length,
		source.once_signal,
...
#ifdef _DEBUG
	printf("Returning %d from pco_pronto_to_ir-signal\n", err);
#endif
	return err;
